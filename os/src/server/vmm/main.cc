...
namespace Genode {
	...
	class Vm {
		...
		public:
			...
			void dump()
			{
				...
				printf("  ip        = %08lx\n", _state->ip);
				printf("  cpsr      = %08lx\n", _state->cpsr);
				for (unsigned i = 0; i < Cpu_state_modes::Mode_state::MAX; i++) {
	...
	class Vmm : public Genode::Thread<8192>
	{
		private:
			...
			{
				PWRN("Vm tried to access %p which isn't allowed", _tsc.last_failed_access());
				_vm->dump();
				return false;
			}
			...
			bool _handle_vm()
			{
				...
				case Cpu_state::SUPERVISOR_CALL:
					...
					break;
				default:
					PERR("Curious exception occurred");
					_vm->dump();
					return false;
				}
				return true;
...
int main()
{
	...
	static Genode::Vmm vmm(TSC_IMX6_BASE, /* TPC_VEA9X4_BASE,  */
	                       SYS_IMX6_BASE, /* SP810_VEA9X4_BASE */
	                       &vm);
